...
# Add a bit of regularization just in case
...
_, pred_src_norm = tps.applyTransformation(ref_norm.reshape(1, -1, 2).astype(np.float32))

# Un-normalize
pred_src = ((pred_src_norm.reshape(-1, 2) + 1.0) / 2.0) * src_scale + src_off

res = np.linalg.norm(mkpts_src - pred_src, axis=1)
print("RMSE forward (normalized):", np.sqrt(np.mean(res**2)))
print("First 5 pred:", pred_src[:5])
print("First 5 true:", mkpts_src[:5])
