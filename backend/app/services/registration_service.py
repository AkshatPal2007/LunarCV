...
def _percentile_stretch(img: np.ndarray, p_low: float = 1.0, p_high: float = 99.0) -> np.ndarray:
...
def run_registration(
    ...
    try:
        ...
        for i in range(n_strips):
            ...
            # LightGlue (primary)
            pts_s, pts_r, conf = lightglue.match(patch_src, patch_ref, conf_threshold=0.0)

            # RIFT2 fallback on sparse strips
            if rift2 is not None and len(pts_s) < RIFT2_THRESHOLD:
                try:
                    r_s, r_r, r_c = rift2.match(patch_src, patch_ref, conf_threshold=0.0)
                    if len(r_s) > 0:
        ...
        mkpts_src_c, mkpts_ref_c, conf_c, _, _ = magsac_filter(
            mkpts_src, mkpts_ref, conf_all,
            model="homography", ransac_reproj_threshold=4.0,
        )

        if len(mkpts_src_c) < 4:
            raise ValueError(f"Insufficient inliers after MAGSAC++ ({len(mkpts_src_c)}).")

        # ── 5. Spatial top-K ─────────────────────────────────────────────────
        mkpts_src_k, mkpts_ref_k, conf_k = spatial_topk_filter(
            mkpts_src_c, mkpts_ref_c, conf_c,
            h_src, w_src, n_rows=4, n_cols=4, top_k_per_cell=3,
        )

        # ── 6. Sub-pixel refinement ──────────────────────────────────────────
        _update(75, "Sub-pixel refinement…")
        mkpts_src_r, mkpts_ref_r, _ = refine_matches(
            src_img, ref_img, mkpts_src_k, mkpts_ref_k,
        )
        ...
        with open(csv_path, "w", newline="") as f:
            w = csv.writer(f)
            w.writerow(["source_x", "source_y", "reference_x", "reference_y", "confidence", "inlier"])
