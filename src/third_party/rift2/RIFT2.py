...
from third_party.rift2.phase_congruency.phasecong import phasecong
...
class RIFT2:
    ...
    def extract_patches(self, img, x, y, s, t):
        ...
        for ch in range(m):
            ...
            ivec = (1 - yp) * (
                xp * imgch.ravel()[ind2] + (1 - xp) * imgch.ravel()[ind1]
    ...
    def orientation(
        self, x, y, gradientImg, gradientAng, patch_size, n, ORI_PEAK_RATIO
    ):
        se = cv2.getStructuringElement(
            cv2.MORPH_ELLIPSE, (int(patch_size + 1), int(patch_size + 1))
        )
        Sa = se.astype(np.uint8)
        hist, max_value = self.calculate_orientation_hist(
        ...
        for k in range(n):
            ...
            if hist[k] > hist[k1] and hist[k] > hist[k2] and hist[k] > mag_thr:
                bin = (
                    k
                    - 1
                    + 0.5 * (hist[k1] - hist[k2]) / (hist[k1] + hist[k2] - 2 * hist[k])
                )
                if bin < 0:
                    bin = n + bin
                elif bin >= n:
    ...
    def process_features(self, img1, img2):
        ...
        kp1 = [
            cv2.KeyPoint(x=float(point[0]), y=float(point[1]), size=1) for point in kp1
        ]
        kp2 = [
            cv2.KeyPoint(x=float(point[0]), y=float(point[1]), size=1) for point in kp2
        ]
