...
from spatial_uniformity import spatial_uniformity_report, spatial_topk_filter
from transform import compute_registration, make_overlay, make_checkerboard
...
def main() -> None:
    ...
    su_final = spatial_uniformity_report(
        mkpts_src_ref, h_src, w_src, label="Sub-pixel refined inliers", n_rows=4, n_cols=4
    )
    ...
    draw_connected_matches(
        ohrc_scaled, lro_norm, mkpts_src_ref, mkpts_ref_ref,
    ...
    print(f"  [diag] Bounding Box           : x=[{x_min}, {x_max}], y=[{y_min}, {y_max}]")
    print(f"  [diag] Output canvas size     : ({out_h}, {out_w})")
    print(f"  [diag] Valid overlap area     : {reg.mask_overlap.sum()} px ({reg.overlap_pct:.1f}% of warped LRO)")

    # 1. Registered image, overlay, checkerboard
    cv2.imwrite(str(SUBMISSION_DIR / "registered.png"), reg.warped_ref)

    overlay, _ = make_overlay(reg.warped_ref, reg.warped_src, reg.mask_ref, reg.mask_src)
    cv2.imwrite(str(SUBMISSION_DIR / "overlay.png"), overlay)

    checker = make_checkerboard(
        reg.warped_ref, reg.warped_src,
        reg.mask_ref, reg.mask_src, reg.mask_overlap,
        grid_size=50,
    )
    ...
    axes[0, 0].imshow(ohrc_scaled,  cmap="gray"); axes[0, 0].set_title("Original OHRC");  axes[0, 0].axis("off")
    axes[0, 1].imshow(lro_norm,     cmap="gray"); axes[0, 1].set_title("Original LRO NAC"); axes[0, 1].axis("off")
    axes[0, 2].imshow(reg.warped_ref, cmap="gray"); axes[0, 2].set_title("Warped LRO");  axes[0, 2].axis("off")
    axes[1, 0].imshow(cv2.cvtColor(overlay, cv2.COLOR_BGR2RGB)); axes[1, 0].set_title("Alpha Overlay"); axes[1, 0].axis("off")
    axes[1, 1].imshow(checker, cmap="gray"); axes[1, 1].set_title("Checkerboard"); axes[1, 1].axis("off")
    axes[1, 2].axis("off")
    fig.tight_layout()
    fig.savefig(FIGURES_DIR / "registration_product_diagnostic.png", dpi=150, bbox_inches="tight")
    plt.close(fig)

    print(f"  [out] Saved registered.png, overlay.png, checkerboard.png -> {SUBMISSION_DIR}")
    print(f"  [out] Saved diagnostic plot -> {FIGURES_DIR / 'registration_product_diagnostic.png'}")
    ...
    with open(csv_path, mode="w", newline="") as f:
        ...
        for pt_s, pt_r in zip(mkpts_src_ref, mkpts_ref_ref):
